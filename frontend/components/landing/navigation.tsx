      ...
      >
        ...
        >
          ...
          <a href="#" className="flex items-center gap-2 group">
            <span className={`font-display tracking-tight transition-all duration-500 ${isScrolled ? "text-xl text-white" : "text-2xl text-white"}`}>Santulan</span>
            <span className={`font-mono transition-all duration-500 ${isScrolled ? "text-[10px] mt-0.5 text-white/50" : "text-xs mt-1 text-white/60"}`}>TM</span>
          </a>
            ...
            {!isSignedIn ? (
              <>
                <SignInButton mode="modal" forceRedirectUrl="/dashboard">
                  <button className={`transition-all duration-500 text-sm text-white/70 hover:text-white`}>
                    Sign in
                  </button>
                </SignInButton>
              ...
              <>
                <Link href="/dashboard" className="text-sm text-white/70 hover:text-white mr-4 transition-colors">
                  Go to Dashboard
                </Link>
                <UserButton />
              </>
            ...
            {!isSignedIn ? (
              <>
                <SignInButton mode="modal" forceRedirectUrl="/dashboard">
                  <Button 
                    variant="outline" 
                    className="flex-1 rounded-full h-14 text-base bg-transparent text-white border-white hover:bg-white/10 hover:text-white"
                    onClick={() => setIsMobileMenuOpen(false)}
                  >
                    Sign in
                  </Button>
